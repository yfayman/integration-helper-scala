...
object NextRunCalculator {
  ...
  protected def calculateTimeToNextTrigger(
    ...
    val nextDate = getNextDate(cronDom, cronDow, fromYear, fromDayOfMonth, fromDayOfWeek, fromMonth)
    ...
    nextExecutionCal.setTimeZone(gmt)
  ...
  def getNextDate(
    ...
    val lastDayOfMonth = fromMonth.getLastDay(year)

    if (!cronDom.isOmmitted) {
      val nextDom = getNextDayOfMonth(cronDom, fromDayOfMonth).get
